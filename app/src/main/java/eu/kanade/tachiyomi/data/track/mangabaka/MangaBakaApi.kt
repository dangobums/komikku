...
class MangaBakaApi(
    ...
    private val client = baseClient.newBuilder().addInterceptor {
        it.request().newBuilder()
            .header(
                ...
                buildString {
                    // dangobums -->
                    append("Komikku/v${BuildConfig.VERSION_NAME} ")
                    append("(${BuildConfig.APPLICATION_ID} ${BuildConfig.COMMIT_SHA}) ")
                    append("(Android) (https://github.com/komikku-app/komikku)")
                    // dangobums <--
                },
            )
            .build()
    ...
    private fun parseIsoDateAsLocalStartOfDay(isoDate: String?): Long? {
        ...
        // dangobums --> java.time instead of kotlinx-datetime
        return isoDate
            ?.substringBefore("T")
    ...
    companion object {
        // Mihon's registered OAuth client; Komikku also handles the mihon:// scheme
